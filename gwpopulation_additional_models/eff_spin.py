...
class Transition_chi_eff:
    ...
    def call(self, dataset, *args, **kwargs):
        ...
        p_chi = (dataset['chi_eff']<m_t) * truncnorm(dataset['chi_eff'], mu_chi_eff_low, np.exp(log_sigma_chi_eff_low), 1, -1)
        p_chi += (dataset['chi_eff']>=m_t) * (xi_chi_eff* self.p_Uniform_chi_eff(dataset['chi_eff', w])+ (1-xi_chi_eff)*truncnorm(dataset['chi_eff'], mu_chi_eff_high, np.exp(log_sigma_chi_eff_high), 1, -1))
